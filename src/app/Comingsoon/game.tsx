...
export default function NotFound() {
  ...
  useEffect(() => {
    ...
    function animate() {
      ...
      const maxDelay = 800;
      currentDelay =
        initialDelay + (maxDelay - initialDelay) * (progress * progress);
  ...
  useEffect(() => {
    if (userChoice && botChoice && result) {
      const soundMap: Record<
        Result,
        { src: string; delay: number; volume: number }[]
      > = {
        win: [
          { src: "/sounds/victory.mp3", delay: 4300, volume: 0.2 },
          { src: "/sounds/Celebration.mp3", delay: 7300, volume: 0.2 },
        ],
        lose: [
    ...
    return (
      <div ref={resultRef} className="relative w-full h-screen overflow-hidden">
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(200.32deg, #000000 -20.61%, #0049FF 184.91%)",
        ...
        >
          ...
          <div className="sm:hidden flex flex-col items-center justify-center absolute right-0 left-0 translate-y-[-130%]">
            <p className="font_regular text-white text-[20px]">
                ...
                {botChoice && (
                  <>
                    ...
                    <Image
                      ...
                      alt="pilihan bot"
                      className="relative xl:w-auto lg:w-[400px] md:w-[330px] sm:w-[250px] w-[60vw] sm:h-auto h-[60vw]"
                    />
                  </>
                )}
              </div>
                ...
                {userChoice && (
                  <>
                    ...
                    <Image
                      ...
                      alt="pilihan user"
                      className="relative xl:w-[100%] lg:w-[400px] md:w-[330px] sm:w-[250px] w-[60vw] sm:mb-0 mb-[-25px] sm:h-auto h-[60vw]"
                    />
                  </>
                )}
              </div>
            </div>
            <h1 className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 font_bold xl:text-[121.32px] lg:text-[100px] md:text-[80px] text-[60px] text-white">
              VS
            </h1>
          </div>

          <div className="sm:hidden flex flex-col items-center justify-center  absolute right-0 left-0 translate-y-[130%]">
            <h1 className="font_bold text-[#FF4900] text-[36px] tracking-tight leading-none">
              You
            </h1>
            <p className="font_regular text-white text-[20px]">
              {labelMap[userChoice]}
          ...
          <div className="hidden sm:flex flex-row items-center justify-between w-full xl:px-40 lg:px-30 md:px-25 sm:px-15 px-10 xl:text-[42.52px] lg:text-[38px] md:text-[30px] text-[20px] absolute translate-y-[500%]">
            <h1 className="font_regular text-white">
              {labelMap[botDisplayChoice]}
            </h1>
  ...
  return (
    <div className="relative flex flex-col items-center justify-center overflow-hidden bg-[#001B5E]">
      ...
      <div className="relative flex flex-col items-center w-full justify-center min-h-screen ">
        <BgGrid
          classNameVertical="object-cover translate-x-[-20%] opacity-40"
          classNameHorizontal="opacity-40 object-cover translate-y-[34%] "
          speedVertical={1}
          speedHorizontal={1}
        />
        <Lock />
        ...
        <div className="flex z-30  flex-col items-center space-y-6 mb-14">
          <div className="hidden md:flex flex-row items-center justify-center z-30 font_bold whitespace-nowrap space-x-6 text-[4rem] sm:text-[5rem] md:text-[6rem] lg:text-[7rem] xl:text-[10rem]">
            <div className="flex flex-row items-center text-[#0049FF]">
              ...
              <Image
                src={n}
                alt="n"
                className="h-10 mt-2 sm:mt-1 md:mt-1.5 lg:mt-1 xl:mt-0 sm:h-14 md:h-16 lg:h-[5rem] xl:h-[8rem] w-auto"
              />
              <span>g</span>
            </div>
            <div className="flex flex-row items-center text-[#0049FF]">
          ...
          <div className="md:hidden z-30 flex flex-col items-center justify-center font_bold text-[3.5rem] leading-none space-y-2">
            <div className="flex flex-row items-center text-[#0049FF]">
              ...
              <Image
                src={n}
                alt="n"
                className="h-[2.5rem] w-auto translate-y-[6px] ml-[2px]"
              />
              <span>g</span>
              <Image
                src={o}
                alt="o"
                className="h-[2.5rem] w-auto translate-y-[6px] ml-4"
              />
              <span>n</span>
            </div>
            <div className="flex flex-row items-center text-[#FF4900]">
          ...
          <h2 className="text-lg hidden md:block text-white font_regular text-center md:text-xl lg:text-2xl xl:text-[42.52px]">
            We’re cooking up something awesome. While we polish up the site,
            lets play a game!
          </h2>
          <h2 className="text-lg md:hidden text-white font_regular font-normal text-center md:text-xl lg:text-2xl xl:text-[42.52px]">
            We’re cooking up something awesome.
          </h2>
        </div>
        {!isOpening && showPlayBtn && (
          <BtnPlay onClick={() => setShowPlayBtn(false)} />
        )}
        <p className="text-[14px] md:hidden w-full mt-2 text-white font-normal text-center fixed bottom-[50px] ">
          While we polish up the site, lets play a game!
        </p>
      ...
      <div className="relative flex flex-col items-center justify-center overflow-hidden min-h-screen bg-[#001B5E] w-full">
        <BgGrid
          classNameVertical="object-cover translate-x-[-20%] opacity-40"
          classNameHorizontal="object-cover translate-y-[34%] opacity-40"
          speedVertical={0.7}
          speedHorizontal={1}
        />
        ...
        <div ref={pickRef} className="relative flex flex-col sm:mb-80 px-4 ">
          ...
          <h2 className="text-white font_regular text-center text-lg sm:text-3xl md:text-[30px] flex flex-row items-center">
            ...
            <span className="font_bold inline-block align-middle mx-1 text-lg sm:text-3xl md:text-[30px]">
              Ready?
            </span>
          </h2>
        </div>
        ...
        <div className="relattive  sm:absolute sm:bottom-[-50px] w-full flex sm:flex-row flex-col items-center justify-center space-x-[-60px] space-y-[-70px] sm:mr-0 mr-12 ">
          <Image
            src={minutes}
            alt="coming soon"
            className="rotate-[167.99deg] translate-y-[70%] md:block hidden z-20 xl:w-[15%] lg:w-[30%] md:w-[20%] "
          />
          <GameCard
            src={gunting}
            alt="Scissors"
          ...
          <Image
            src={orange}
            alt="coming soon"
            className="rotate-[196.58deg] translate-y-[50%] xl:w-[15%] lg:w-[30%] md:w-[20%] md:block hidden z-20"
          />
        </div>
      </div>
    </div>
