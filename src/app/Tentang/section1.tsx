...
export default function Section1() {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.from(".anim-from-left", {
        ...
        scrollTrigger: { trigger: ".anim-from-left", start: "top 85%", end: "bottom 15%", toggleActions: "restart reverse restart reverse" },
      });
      gsap.from(".anim-from-right", {
        x: 100, opacity: 0, duration: 1.5, ease: "power3.out",
        scrollTrigger: { trigger: ".anim-from-right", start: "top 85%", end: "bottom 15%", toggleActions: "restart reverse restart reverse" },
      });
      gsap.from(".anim-from-bottom", {
        y: 100, opacity: 0, duration: 1.2, ease: "power3.out", stagger: 0.2,
        scrollTrigger: { trigger: containerRef.current, start: "top 70%", end: "bottom 15%", toggleActions: "restart reverse restart reverse" },
      });
      gsap.from(".anim-decor-left", {
        x: -100, opacity: 0, duration: 1.5, stagger: 0.2, ease: "power3.out",
        scrollTrigger: { trigger: ".section-2-container", start: "top 80%", end: "bottom 15%", toggleActions: "restart reverse restart reverse" },
      });
      ...
      const tl = gsap.timeline({
        scrollTrigger: {
          ...
          onEnter: () => gsap.delayedCall(1, () => document.querySelectorAll('.pupil').forEach(el => el.classList.add('is-animating'))),
          onLeaveBack: () => document.querySelectorAll('.pupil').forEach(el => el.classList.remove('is-animating')),
          onEnterBack: () => gsap.delayedCall(1, () => document.querySelectorAll('.pupil').forEach(el => el.classList.add('is-animating'))),
          onLeave: () => document.querySelectorAll('.pupil').forEach(el => el.classList.remove('is-animating')),
        },
      });
  ...
  return (
    <div ref={containerRef} className="w-full flex flex-col items-center justify-center bg-gradient-to-b from-[#a1c0e3] to-[#FFFFFF] overflow-x-hidden">
      <Image
        src={bg_grid}
      ...
      <div className="relative w-full aspect-[1700/755] overflow-hidden">
        ...
        <div className="z-30 absolute bottom-[17%] left-[13.3%] anim-from-bottom">
          <ChatButton size="clamp(1.5vw, 1.5vw, 1.5rem)">Mari berkenalan!</ChatButton>
        </div>
