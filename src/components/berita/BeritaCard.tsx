...
export default function BeritaCard({ title, date, description, color, imageUrl, maxWords = 25, onClick }: BeritaCardProps) {
  return (
    <div
      onClick={onClick}
      onKeyDown={(e) => {
      ...
      className={`
        ...
        ${color === 'orange' ? 'bg-[#FF4500] border-[#FF4500] hover:border-orange-300' : 'bg-[#0062FF] border-[#0062FF] hover:border-blue-300'}
    ...
    >
      ...
      <div className="mx-8 mt-8 overflow-visible relative">
        <div className="inverted-radius-big">
          {imageUrl ? (
            <img src={getFullImageUrl(imageUrl)} alt={title} className="object-cover w-full h-full" loading="lazy" draggable={false} />
          ) : (
        ...
        <div className="absolute" style={{ right: '2px', bottom: '-10px' }}>
          <div
            ...
            ${color === 'orange' ? 'border-[#FF4500] group-hover:border-orange-300' : 'border-[#0062FF] group-hover:border-blue-300'}
            transition
          `}
          >
