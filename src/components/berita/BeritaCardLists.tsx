...
import BeritaEMagazine from "@/components/berita/BeritaEMagazine";
...
export default function BeritaSwiperSection() {
  ...
  let newsList = data?.listNews?.news ?? [];
  ...
  if (category !== "Semua") {
    newsList = newsList.filter((item: any) => item.categoryName === category);
  }
  if (search.trim() !== "") {
  ...
  // Swiper progress bar calculation
    ...
    <section className="w-full flex flex-col min-h-screen pb-36">
      ...
      <div className="w-full max-w-[1450px] mx-auto flex flex-col md:flex-row md:justify-between md:items-center gap-5 md:gap-0 mb-12 px-6">
        ...
        <div className="relative w-full md:w-auto max-w-[340px]">
          ...
          <span className="pointer-events-none absolute right-8 top-1/2 -translate-y-1/2 text-blue-500 text-xl">
            ▼
          </span>
        </div>
        {/* Search box */}
