      ...
      {expressions
        ...
        .map((exp) => (
          <div key={exp} onClick={() => onSelect(exp)} className="cursor-pointer flex flex-col items-center w-[120px] md:w-[140px]">
            <div className="w-[100px] h-[100px] md:w-[150px] md:h-[150px] relative mb-2">
              <Image src={expressionImageMap[exp]} alt={`Ekspresi Mori ${exp}`} fill className="object-contain" />
            </div>
            <div className="bg-white border-2 border-[#FF4900] text-[#FF4900] text-base md:text-xl font-bold px-6 py-1 md:px-8 md:py-2 md:-translate-y-14 rounded-full shadow-sm text-center">{exp.charAt(0).toUpperCase() + exp.slice(1)}</div>
          </div>
        ))}
    </div>
  );
