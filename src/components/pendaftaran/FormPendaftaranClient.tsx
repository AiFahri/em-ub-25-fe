...
export default function FormPendaftaranClient() {
  ...
  const fields: Field[] = useMemo(() => {
    return [...(workProgram?.form?.fields || [])].sort(
      (a, b) => a.order - b.order
    );
  }, [workProgram]);
  ...
  const [modalMode, setModalMode] = useState<"confirm" | "success" | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    (savedAnswers as Answer[]).forEach((answer) => {
      ...
      if (field.type === "UPLOAD" && answer.value?.[0]) {
        ...
        restored[key] = Array.isArray(answer.answerOptionsID)
          ? answer.answerOptionsID.length === 1
            ? answer.answerOptionsID[0]
            : answer.answerOptionsID.map(String)
          : String(answer.answerOptionsID);
      } else if (field.type === "TEXT") {
        if (answer.value && answer.value.length > 0) {
          restored[key] =
            answer.value.length > 1 ? answer.value : answer.value[0];
        }
      }
    });
  ...
  if (error || !workProgram || !fields)
    return (
      <p className="text-white text-center py-10">Gagal memuat formulir.</p>
    );
    ...
    <div className="min-h-screen bg-[#0049FF] px-4 sm:px-6 md:px-10 py-10 flex justify-center">
      <div className="w-full md:max-w-4xl lg:max-w-7xl flex flex-col gap-y-10">
        <div className="bg-[#E6EDFF] mt-24 mb-5 rounded-[30px] shadow-lg w-full flex flex-col md:flex-row gap-y-5 overflow-hidden">
          <div className="w-full md:w-4/7 py-10 flex flex-col items-center justify-center px-6 md:px-10 lg:px-20">
            {imageUrl ? (
              <Image
                src={`https://is3.cloudhost.id/emub/${imageUrl.replace(
                  /^\/+/,
                  ""
                )}`}
                alt="Poster"
          ...
          <div
            className="w-full md:w-3/7 py-10 px-6 sm:px-10 md:px-10 lg:px-20 bg-white z-20 rounded-t-[30px] md:rounded-tr-[0] md:rounded-bl-[30px] flex flex-col"
            style={{ filter: "drop-shadow(0 15px 15px rgba(0, 0, 0, 0.30))" }}
          >
            <h2 className="text-center text-xl md:text-4xl font-bold text-[#002787] mb-4">
              {workProgram?.title}
            </h2>

            <div
              className="flex-1 overflow-y-auto custom-scroll pr-5"
              style={{ maxHeight: "650px" }}
            >
              <div>
                {workProgram?.form?.description || "Deskripsi tidak tersedia"}
              </div>
            </div>

            {workProgram?.form?.fileLink && (
              <a
                href={workProgram.form.fileLink}
                target="_blank"
                rel="noopener noreferrer"
                className="mt-4 text-[#0538B9] font-bold underline text-sm sm:text-base md:text-lg flex gap-2 hover:text-blue-800 self-center md:self-start"
              >
                <svg
        ...
        <div className="bg-white rounded-[30px] shadow-lg w-full px-6 md:px-10 lg:px-20 py-10">
          <h2 className="text-[#002787] text-xl sm:text-2xl md:text-4xl font-bold mb-6 text-center">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                ...
                case "CHECKBOX":
                  return (
                    <FieldCheckboxGroup
                      ...
                      value={
                        Array.isArray(formData[String(field.id)])
                          ? (formData[String(field.id)] as string[])
                          : []
                      }
                  ...
                  return (
                    <FieldUpload
                      ...
                      value={
                        ...
                        typeof formData[key][0] === "object"
                          ? (formData[key] as FileWrapper[])
                          : []
                      }
                      onChange={handleChange}
            ...
            <div className="pt-6 flex justify-center">
              <button
                type="submit"
                className="px-10 py-2 rounded-2xl bg-[#004DFF] hover:bg-[#003bd1] text-white font-semibold text-lg"
              >
                Daftar
              </button>
            </div>
      ...
      {modalMode && (
        <ModalSubmit
          ...
          onConfirm={modalMode === "confirm" ? handleConfirmSubmit : undefined}
          groupLink={
            modalMode === "success" ? workProgram?.form?.groupLink : undefined
          }
          slug={slug}
        />
      )}
