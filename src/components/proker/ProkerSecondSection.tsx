...
const ProkerSecondSection: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const timer5 = setTimeout(() => setAnimationPhase(5), 3000); // 3s
    const timer6 = setTimeout(() => setAnimationPhase(6), 3500); // 3.5s
    const timer7 = setTimeout(() => setAnimationPhase(7), 4000);
  ...
  return (
    <section
      ...
      className="relative w-full flex items-center justify-center md:min-h-screen py-8 md:py-16 overflow-hidden"
    >
      <style>{`
                @keyframes shake {
      ...
      <div className="flex items-center justify-center gap-6 md:gap-10 lg:gap-16">
        ...
        <div className="relative w-40 h-40 md:w-56 md:h-56 lg:w-80 lg:h-80">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full">
            <Image src={Assets} alt="Background Asset Group" layout="responsive" width={200} height={200} />
          </div>
          <div className={`absolute top-1/2 left-1/2 w-[45%] transition-transform duration-1000 ease-in-out ${getOrangeTransform()} ${getOrangeAnimation()}`}>
            <Image src={Orange} alt="Orange Asset" layout="responsive" width={90} height={90} />
          </div>
        </div>
