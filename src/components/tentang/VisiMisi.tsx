...
const VisiMisi: FC = () => {
  ...
  const progressValue: MotionValue<number> = useTransform(
    scrollYProgress,
    [0, 1],
    [0, 100]
  );
  ...
  const pupilX = Math.max(
    -maxPupilMovement,
    Math.min(maxPupilMovement, rawPupilX)
  );
  const pupilY = Math.max(
    -maxPupilMovement,
    Math.min(maxPupilMovement, rawPupilY)
  );

  return (
    <section
      ref={sectionRef}
      className="relative w-11/12 mx-auto py-24 bg-white"
    >
            ...
            <div className="relative w-full h-48">
              ...
              >
                <div className="text-[#0049FF]">
                  Mewujudkan Eksekutif Mahasiswa Universitas Brawijaya 2025
                  sebagai Simpul Memori yang{" "}
                  <span className="font-bold">Inklusif</span> dan{" "}
                  <span className="font-bold">Progresif</span> dalam Harmonisasi
                  Cita Karakter Brawijaya
                </div>
              </motion.div>
